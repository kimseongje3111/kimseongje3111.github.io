...
그다음 ***JVM*** 내 로드된 바이트코드는 ***Execution Engine***에 의해 기계어로 해석되어 메모리 상(Runtime Data Area)에 배치된다. ***Execution Engine***은 인터프리터(Interpreter)와 ***JIT(Just In Time)***컴파일러(Compiler)로 구성되어 있다. 앞서 말한 코드가 ***JVM***을 통해 해석되기 때문에 OS으로부터 직접 제어 받는 방식보다 속도 면에서 느리다는 단점을 보완하기 위해 ***JIT*** 컴파일러가 존재한다.
...
> 상수풀(Constant Pool) : Type에서 사용된 상수들을 저장하는 곳이다.(상수의 중복이 생기면 기존의 상수를 사용한다.) 쉽게 String Type의 상수에 대해 예를 들면 직접 new를 이용하여 String 객체를 생성(힙에 생성) 하지 않고 ""으로 생성한 두 변수의 상수값이 같다면 같은 주솟값을 가진다. (String s1 = "hello", String s2 = "hello" --> s1과 s2는 같은 주솟값.) 그 외 final class 변수들도 상수 풀에 값을 복사한다.
