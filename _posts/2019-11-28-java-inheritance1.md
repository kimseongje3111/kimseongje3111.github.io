...
상속의 구현은 간단하다.
위 결과 하위 클래스인 B는 상위 클래스 A의 필드와 메서드를 상속받는다.
...
위 코드를 각 클래스에 추가하였다.
ChangeInfo 메서드를 상위 클래스에 추가하고 하위 클래스 Manager에서 이를 오버라이딩하였다.
...
그와 반대로 Intern 클래스는 메서드 오버라이딩을 하지 않아 상위 클래스인 Employee의 메서드를 사용하기 때문에 'Not Available'이 출력된다.
