...
이제 ***HashMap***이 ***Map*** 인터페이스의 구현체인 것을 알았다.
...
해시 버킷의 개수(***M***)가 항상 2의 제곱수이기 때문에 '***X.hashcode() % M***'를 계산할 때 ***X.hashCode()***의 32비트 중 하위 2의 승수개의 비트만 사용하게 되어 해시 충돌이 자주 발생할 가능성이 있다.
...
Java 8에서는 Java 7보다 개선되고 단순한 형태의 보조 해시 함수를 가진다.
...
위 내용에서 다루지 않았지만 해당 내용에 대한 자세한 설명은 아래 참조 사이트와 Horner’s method를 참고하시면 좋을 것 같습니다.
